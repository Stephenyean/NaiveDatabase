...
RC RM_Manager::createFile(const char * filename, int recordSize)
{
	...
	fileHead->maxRecordNumPerPage = int((PAGE_SIZE - sizeof(File_Head)) / (recordSize + 0.125));
...
RC RM_FileHandle::getRec(const RID &rid, RM_Record &rec)
{
	...
	int pageNum, slotNum;
	rec.recordSize = fileHead->recordSize;
	...
	char* newData = new char[fileHead->recordSize];
	memcpy(newData, data, fileHead->recordSize);
	rec.setData(newData);
	rec.setRid(rid);
	return OK;
}
...
RC RM_FileHandle::insertRec(const char *pData, RID &rid)
{
	...
	memcpy(data, pData, fileHead->recordSize);
	// modify pageHead
	pageHead->numRecords++;
	...
	if (verbose == 2)
	{
		cout << "[info] insert Record" << pData << endl;
	}
	// set markDirty
...
RC RM_FileHandle::updateRec(const RM_Record &rec)
{
	...
	memcpy(data, rec.pData, fileHead->recordSize);
	bpm->markDirty(index);
	if (verbose == 2)
...
{
	rec.recordSize = fileHandle.fileHead->recordSize;
	while (true)
	{
		...
		{
			char* data = (char*)b + fileHandle.getOffset(currentSlot) + attrOffset;
			if (attrtype == DINT)
			{
